...
class SubmissionRepresentationNormalizer extends SerializerAwareNormalizer implements DenormalizerInterface, NormalizerInterface
{
    ...
    {
        if ($type !== 'BenBorla\\JobAdder\\V2\\Model\\SubmissionRepresentation') {
            return false;
        }
    ...
    {
        if ($data instanceof \BenBorla\JobAdder\V2\Model\SubmissionRepresentation) {
            return true;
        }
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        $object = new \BenBorla\JobAdder\V2\Model\SubmissionRepresentation();
        ...
        if (property_exists($data, 'jobApplication')) {
            $object->setJobApplication($this->serializer->deserialize($data->{'jobApplication'}, 'BenBorla\\JobAdder\\V2\\Model\\JobApplicationSummaryModel', 'raw', $context));
        }
        if (property_exists($data, 'contacts')) {
            $values = [];
            foreach ($data->{'contacts'} as $value) {
                $values[] = $this->serializer->deserialize($value, 'BenBorla\\JobAdder\\V2\\Model\\ContactSummaryModel', 'raw', $context);
            }
        ...
        if (property_exists($data, 'links')) {
            $object->setLinks($this->serializer->deserialize($data->{'links'}, 'BenBorla\\JobAdder\\V2\\Model\\SubmissionLinks', 'raw', $context));
        }
