...
class EmploymentModelNormalizer extends SerializerAwareNormalizer implements DenormalizerInterface, NormalizerInterface
{
    ...
    {
        if ($type !== 'BenBorla\\JobAdder\\V2\\Model\\EmploymentModel') {
            return false;
        }
    ...
    {
        if ($data instanceof \BenBorla\JobAdder\V2\Model\EmploymentModel) {
            return true;
        }
    ...
    public function denormalize($data, $class, $format = null, array $context = [])
    {
        ...
        $object = new \BenBorla\JobAdder\V2\Model\EmploymentModel();
        if (!isset($context['rootSchema'])) {
            $context['rootSchema'] = $object;
        }
        if (property_exists($data, 'current')) {
            $object->setCurrent($this->serializer->deserialize($data->{'current'}, 'BenBorla\\JobAdder\\V2\\Model\\CurrentEmploymentModel', 'raw', $context));
        }
        if (property_exists($data, 'ideal')) {
            $object->setIdeal($this->serializer->deserialize($data->{'ideal'}, 'BenBorla\\JobAdder\\V2\\Model\\IdealEmploymentModel', 'raw', $context));
        }
